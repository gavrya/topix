...
interface TopicHandlerProps<A extends Action = AnyAction, S extends State = any> {
    action: A;
    state: S;
    emit: Emit<A>;
}
...
interface Module<N extends string = string, A extends Action = AnyAction, S extends State = any, AT extends ActionTypes = ActionTypes, AC extends ActionCreators<A> = ActionCreators<A>> {
    namespace: N;
    initialState: S[N];
    topics: Topic<A, S>[];
    actionTypes: AT;
